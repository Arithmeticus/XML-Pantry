...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="xsl:variable[@name = 'my-complicated-map']/xsl:map"
        use-when="$refactor-var-my-complicated-map">
        ...
        <xsl:copy>
            ...
            <xsl:for-each-group select="$database//*[@xml:id]" group-by="@xml:id">
                <xsl:if test="count(current-group()) gt 1">
                    <xsl:message
                        select="'There are ' || string(count(current-group())) || ' entries for ' || current-grouping-key() || '; retrieving only first one.'"
                    />
                </xsl:if>
                <xsl:element name="xsl:map-entry" namespace="http://www.w3.org/1999/XSL/Transform">
                    <xsl:attribute name="key">
                        <xsl:value-of
                            select="codepoints-to-string(39) || current-grouping-key() || codepoints-to-string(39)"
                        />
                    </xsl:attribute>
